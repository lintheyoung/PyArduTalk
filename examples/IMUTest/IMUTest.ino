...
void handleRequest(byte requestType) {
    ...
    switch (requestType) {
        ...
        case PyArduTalk::TYPE_GYRO:
            {
                // 在实际应用中，这里应该读取真实的陀螺仪数据
                // 这里使用模拟数据演示
                float yaw = 45.67;    // 示例值
                float roll = -12.34;   // 示例值
                float pitch = 89.01;  // 示例值
                
                Serial.println("发送陀螺仪数据响应:");
                Serial.print("Yaw: "); Serial.print(yaw);
                Serial.print(", Roll: "); Serial.print(roll);
                Serial.print(", Pitch: "); Serial.println(pitch);
                
                pyArduTalk.sendGyro(yaw, roll, pitch);
            }
            break;
...
void handleGyro(float yaw, float roll, float pitch) {
    Serial.print("回调 - 接收到陀螺仪数据: ");
    Serial.print("Yaw: "); Serial.print(yaw);
    Serial.print(", Roll: "); Serial.print(roll);
    Serial.print(", Pitch: "); Serial.println(pitch);
}
...
void setup() {
    ...
    pyArduTalk.onGyroReceived(handleGyro);  // 添加陀螺仪回调
